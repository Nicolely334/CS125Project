...
import { logSong, getPresetTags, getUserCustomTags, createTag, type PresetTag, type Tag } from '../services/logs';
import type { Track } from '../services/api';

interface LogSongModalProps {
  track: Track;
  onClose: () => void;
  onSuccess: () => void;
}

export function LogSongModal({ track, onClose, onSuccess }: LogSongModalProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to log song');
    } finally {
      setLoading(false);
    }
  }
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content" onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ marginBottom: '1rem' }}>
          <strong>{track.track}</strong> by <strong>{track.artist}</strong>
        </div>
            ...
            <div className="star-rating">
              {[1, 2, 3, 4, 5].map((star) => (
                <button
                  key={star}
                  type="button"
                  className={`star ${rating !== undefined && star <= rating ? 'filled' : ''}`}
                  onClick={() => {
                    if (rating === star) {
                      setRating(undefined);
                    } else {
